...
class CoordinateFeatureFusionModule(BaseModule):
    ...
    def forward(self, spatial_inputs, context_inputs):
        ...
        x = self.conv0(inputs)
        n, c, h, w = x.size()
        x_atten_h = self.gap_h(x)
        x_atten_w = self.gap_w(x).permute(0, 1, 3, 2)
        x_cat_conv = self.cat_conv(torch.cat([x_atten_h, x_atten_w], dim=2))
        x_split_h, x_split_w = torch.split(x_cat_conv, [h, w], dim=2)
        x_atten_h = self.conv1x1_h(x_split_h)
        x_atten_w = self.conv1x1_w(x_split_w.permute(0, 1, 3, 2))
        # attn = self.attention(x)
        if self.neck:
            x_attn =x*x_atten_h+x*x_atten_w
        else:    
            x_attn = x * x_atten_h * x_atten_w
        return x_attn + x
...
class CoordinateAttentionRefinementModule(BaseModule):
    ...
    def __init__(
        ...
        # self.weight=nn.Parameter(torch.FloatTensor(.5),requires_grad=True)
    def forward(self, x):
        x = self.conv_layer(x)
        # n, c, h, w = x.size()
...
class RepSTDCContextPathNet(BaseModule):
    ...
    def __init__(
        ...
        for channels in last_in_channels:
            if ca_type:
                self.arms.append(
                    CoordinateAttentionRefinementModule(channels, out_channels)
                )
            else:
